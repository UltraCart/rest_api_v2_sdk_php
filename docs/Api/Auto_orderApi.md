...
$api_instance = new ultracart\v2\Api\Auto_orderApi(new \Http\Adapter\Guzzle6\Client());
...
try {
    ...
    echo 'Exception when calling Auto_orderApi->getAutoOrders: ', $e->getMessage(), PHP_EOL;
}
?>
```
...
try {
    ...
    echo 'Exception when calling Auto_orderApi->updateAutoOrder: ', $e->getMessage(), PHP_EOL;
}
?>
```
